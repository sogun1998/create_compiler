...
Token* readConstChar(void) {
  ...
  if(count>1){
    ...
    if(charCodes[currentChar]!= CHAR_SINGLEQUOTE)
      error(ERR_END_OF_COMMENT,token->lineNo,token->colNo);
  }else{
    if(charCodes[currentChar]!= CHAR_SINGLEQUOTE)
      error(ERR_END_OF_COMMENT,token->lineNo,token->colNo);
  }
  ...
  //   return token;
  // } else {
  //   while ((currentChar != EOF) && ((charCodes[currentChar] == CHAR_DIGIT)||(charCodes[currentChar] == CHAR_LETTER)||(charCodes[currentChar] == CHAR_SPACE))) {
  //   token->string[count++] = (char)currentChar;
  //   readChar();
  // }
